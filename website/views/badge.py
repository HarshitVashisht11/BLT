...
def issue_badge(request, repo_name, issue_number):
    ...
    # Get the issue
    try:
        github_issue = GithubIssue.objects.filter(issue__url__contains=f"/{repo_name}/issues/{issue_number}").first()
        ...
        view_count = IP.objects.filter(issue=issue, created__gte=thirty_days_ago).count()
